...
function sendRoleState() {
    game.clientList.forEach((client) => {
        io.to(client.connection).emit(game.roles.map((val) => val === ''), client.role)
    })
}

function sendBoardUpdate() {
    let redLeft = game.boardState.filter((card) => card.revealedColor !== card.borderColor && card.borderColor === RED).length;
    let bluLeft = game.boardState.filter((card) => card.revealedColor !== card.borderColor && card.borderColor === BLU).length;
...
io.on("connection", function (socket) {
    ...
    socket.on("guessed", word => {
        ...
        game.boardState.forEach(card => {
            if (card.word === word) {
                ...
                } else if (game.currentTurn.startsWith("b") && card.borderColor === BLU) {
                    game.guessesRemaining--;
                } else {
    ...
    socket.on("allSelected", function () {
        let allSel =
            game.roleState["bdetective"] &&
            game.roleState["rdetective"] &&
            game.roleState["bspymaster"] &&
            game.roleState["rspymaster"];
...
function setRoleAndName(socketID, role, name) {
    game.clientList.forEach(function (cl) {
        if (cl.connection === socketID) {
            cl.role = role;
            cl.name = name;
            game.roleState[role] = socketID;
        }
    });
}
